  ...
  const SCHEMA = [
    ...
    },
    {
      ...
      items: [
        ...
        { key: 'goalCountdownSec', type: 'number', name: '↳ Обратный отсчёт перед отправкой, сек', desc: 'В режиме агента перед авто-отправкой показывается «через 3, 2, 1…», чтобы успеть вмешаться (⏸). 0 — отправлять сразу.', min: 0, max: 30, requires: 'goalAgentEnabled' },
        { key: 'goalContextTokens', type: 'number', name: '↳ Контекстное окно модели, токены', desc: 'Сколько токенов у выбранной модели. gpt-4o ≈ 128000, o3 ≈ 200000, gpt-4.1 ≈ 1000000.', min: 4000, max: 2000000, requires: 'goalAgentEnabled' },
        { key: 'goalReinjectPercent', type: 'number', name: '↳ Переотправлять инструкцию при, % контекста', desc: 'Экономия токенов: агенту полная инструкция+цель повторяется не каждый ход, а когда история достигает этого % окна. Между этим — компактные подсказки.', min: 1, max: 100, requires: 'goalAgentEnabled' },
      ],
    },
    {
      title: 'AI Finished',
